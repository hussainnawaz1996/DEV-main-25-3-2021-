...
class ScheduledPaymentsView: UIView {
    ...
    func configureView(controller: UIViewController) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(refreshScreen), name: .REFRESH_SCHEDULED_PAYMENT_SCREEN, object: nil)
...
extension ScheduledPaymentsView : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        Router.shared.openSchedulePaymentReceiptViewController(requestModel: requestModel, customModel: customModel, isEditFlow: true, controller: controller!)
    }
    
}

extension ScheduledPaymentsView {
            ...
            DispatchQueue.main.async {
                ...
                case .success(let responseModel):
                    if responseModel.isError{
                        ...
                        self.billPaymentList = list.filter({$0.status == PaymentStatus.logged.rawValue || $0.status == PaymentStatus.inprogress.rawValue || $0.status == PaymentStatus.Scheduled.rawValue})
